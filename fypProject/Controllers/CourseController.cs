...
namespace fypProject.Controllers
{
    public class CourseController : ApiController
    {
        ...
        [HttpGet]
        [Route("api/course/get_all_courses")]
        public HttpResponseMessage GetCourses(int page = 1, int pageSize = 10, bool getAll = false)
        {
            try
            {
                ...
                // ✅ If frontend wants all data
                if (getAll)
                {
                    var allCourses = query
                        .OrderBy(c => c.title)
                        .ToList();
        ...
        public IHttpActionResult SearchCourses(string search)
        {
            ...
            // Case-insensitive search on course name or code
            // Search courses including department
            var courses = (from c in db.courses
                           join d in db.departments
                           on c.department_id equals d.id
                           where c.title.Contains(search) || c.course_code.Contains(search)
                           select new
                           {
                               c.id,
                               c.title,
